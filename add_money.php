...
$db = new DbManage();
$money_add = trim($_POST["a"]);
date_default_timezone_set("Asia/Shanghai");
$time = date("Y-m-d H:i:s");
$sqlTxt1 = "select money from user_info where User_name = '".$_SESSION['username']."'";
$result1 = $db->executeSqlTxt($sqlTxt1);
$row = mysqli_fetch_array($result1);
$money=$row["money"]+$money_add;
$sqlTxt="UPDATE user_info SET money = '".$money."' WHERE User_name = '".$_SESSION['username']."'";
$result = $db->executeSqlTxt($sqlTxt);
if($result){
    $sqlTxt = "INSERT INTO add_money (money_add,User_name,time)VALUES ('".$money_add."','".$_SESSION['username']."','".$time."')";
    $result = $db->executeSqlTxt($sqlTxt);
